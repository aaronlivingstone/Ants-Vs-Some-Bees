...
Name: Aaron Feldman, Farin Namdari
Login: cs61a-ms, cs61a-mr
TA: Phillip C
Section: 27
"""
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while hasattr(nearest_place, 'enterance'):
            if nearest_place.bees != []:
                list_bees = list(nearest_place.bees)
                if count >= self.min_range and count <= self.max_range:
                    return random_or_none(list_bees)
            nearest_place = nearest_place.enterance
            count += 1
        return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        "*** YOUR CODE HERE ***"
        fire_ant_place_before_death_list = list(self.place.bees) #lol ...
        Insect.reduce_armor(self, amount)
        if self.armor <=0:
            for beez in fire_ant_place_before_death_list:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, colony):
        ...
        queen_place(self.place, colony)
        if colony.num_queens > 1:
            self.place.remove_insect(self)
            colony.num_queens -= 1
        else:
            while queen_place != None:
                if queen_place.ant is not None:
...
class queen_place(Place):
    def __init__(self, place, colony):
        self.colony_loc = colony
        self.queen_loc = place
        self.ant = None

    def add_insect(self, insect):
        Place.add_insect(self, insect)
        self.colony_loc.num_queens += 1

    @property
    def bees(self):
        if len(self.colony_loc.bees) > 0:
            return self.colony_loc.bees
        else:
            return self.queen_loc.bees
